    ...
    <div class="period">
        <v-card>
            ...
            <v-row class="ma-0 pb-2 px-5">
                <v-col>
                    <h1>
                        <v-select @change="onClickPeriod()" v-model="period" label="Select a Period" :items="periods"
                            item-text="name" persistent-hint return-object single-line></v-select>
                    </h1>
                </v-col>
                <v-col>
                    <v-btn small color="error" @click=remove(period) class="mt-6">
                        Delete
                    </v-btn>
                </v-col>
            </v-row>
            ...
            <b-modal id="modal-period" title="Period">
                <v-text-field v-model="period.name" outlined label="Period Name" placeholder="Period Name" hide-details
                    class="mb-3"></v-text-field>
                <v-col>
                    <h1>
                        <v-select v-model="period.state" label="Select a Fatura State" :items="faturaStateOptions"
                            item-text="state" persistent-hint return-object single-line></v-select>
                        <v-select label="Select an Hourly Wage to be added to period" :items="hourlyWages"
                            v-model="period.hourlyWage" item-text="name" return-object></v-select>
                    </h1>
                </v-col>
                <template #modal-footer>
                    <div class="float-right">
                        <b-button variant="success" size="sm" @click="save(),olustur()">
                            KAYDET
                        </b-button>
                        <b-button variant="primary" size="sm" @click="olustur()">
                            YENİ
                        </b-button>
            ...
            <b-modal id="modal-scrollable" title="Fatura">
                ...
                <template #modal-footer>
                    <div class="w-100">
                        ...
                        <b-button @click="$bvModal.hide('modal-scrollable')" variant="danger" style="float: right">Hayır
                        </b-button>
                    </div>
